"""
Evader Agent for Competitive MARL
...
class EvaderAgent:
    """
    Evader (target) wrapper for competitive training.

    Uses SAC (Soft Actor-Critic) for continuous control.
    Has opposite reward structure from pursuer.
    """
    ...
    def predict(self, observation: np.ndarray, deterministic: bool = False) -> np.ndarray:
        ...
        Args:
            observation: Environment observation (from evader's perspective)
    ...
    def save(self, path: str):
        ...
        print(f"Evader model saved to: {path}")

    def load(self, path: str):
        """Load model from file."""
        if os.path.exists(path):
            self.model = SAC.load(path, env=self.env)
            print(f"Evader model loaded from: {path}")
        else:
